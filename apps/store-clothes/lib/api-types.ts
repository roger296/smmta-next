...
export interface ThinVariant {
  ...
  /** Multi-axis attributes (e.g. `{ size, colour }`). Optional for
   *  back-compat with legacy single-axis rows. */
...
export interface GroupListItem {
  ...
  seoDescription: string | null;
  sortOrder: number;
  /** Variant axes — `['colour']` for filament, `['size', 'colour']` for
   *  clothing. The storefront uses this to know which selectors to
   *  render. */
