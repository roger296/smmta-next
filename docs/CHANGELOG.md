...
## Prompt 15 (follow-up #12) — checkouts.mollie_payment_id unique transfer + applied 0012 strict-mode fix

- Patch 0011 (mollie_payments upsert) landed and the conflict moved one table over: `checkouts_mollie_payment_id_unq` now fires when the second `db.update(checkouts).set({ molliePaymentId: payment.id })` call assigns a payment id that another checkout already holds. Wrap the assignment in a transaction that first NULLs `molliePaymentId` on any *other* checkout pointing at this payment, then claims it for the current one. Same transaction so a concurrent reader can't observe a state where no checkout owns the binding.
- Patch 0012 (strict-mode locator on the /track page assertion) didn't make it onto origin in the previous round and is still the actual cause of test 1 failing. Replayed it in the same commit: `page.getByText('Confirmed', { exact: true })` instead of `/Confirmed|Picked/i`. The retries triggered the duplicate-key cascade above, which is why the earlier signal looked the same — but the strict-mode error was always the proximate cause.

## Prompt 15 (follow-up #13) — cancelled-test uses the actual mock payment id

- After patch 0013, 7/9 e2e attempts pass: happy-path on first attempt, webhook-fails sad-path on first attempt. Only the cancelled sad-path is still failing — the page is stuck on "Status: PAYING (Mollie: open)" 15 s after the webhook is supposedly fired. Cause: the test hard-codes `fireWebhook(baseURL!, 'tr_mock_1')`, but the mock's `nextPaymentId` doesn't reliably reset to 1 between Playwright workers/tests — by the time the cancelled test runs, the storefront's actual checkout has `mollie_payment_id` of `tr_mock_N` for some N>1. The webhook fires for `tr_mock_1`; the storefront's `finalizeFromMollie` finds no checkout bound to that id and returns early without updating the active checkout, so polling keeps reporting PAYING.
