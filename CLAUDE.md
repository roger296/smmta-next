...
- **Delivery is charged per parcel** (`apps/api/src/modules/storefront/delivery.ts`): each supplier's items make one parcel charged at `suppliers.delivery_charge_gbp` (or the storefront's `STORE_DEFAULT_SHIPPING_GBP` when unset), warehouse items one parcel at the standard rate. The Clothes Shop sends its standard rate with the reservation; the reservation stores the quote in `metadata.delivery`, and `commitOrder` uses that total over the storefront's `deliveryCharge`. `POST /storefront/delivery-quote` gives the same quote before checkout. The Filament Store sends no rate, so it keeps its flat charge.
...
- A second, supplemental feed per shop (`<channel>-stock.xml`, hourly at :15 via `google-feed-stock-build`) carries only id, price and availability, built from the same rows and filters as the nightly one. Google matches on the id; an unknown id is ignored. It exists because Google suspends accounts whose feed disagrees with the shop and a full Ralawise stock sweep takes ~7 hours.
...
- **The admin's menu is trimmed per deployment with `VITE_HIDDEN_SECTIONS`** (build-time, comma-separated section keys = route path without the slash; `apps/web/src/lib/nav-sections.ts`). Navigation only: the routes stay.
- **Shipping labels are bought by the worker on `order.paid` (storefront only) and, when `SHIPPING_LABEL_ON_ALLOCATION` is on, on `order.allocated`**, which `OrderService.allocateStock` emits once every line has stock. The rule is `apps/api/src/modules/shipping/label-trigger.ts`; `ShippingLabelService.requestLabel` never buys twice, so an order that is paid and later allocated gets one label.
- **Products come in by file too.** Products → Import CSV (`POST /api/v1/products/import-csv`) reads the product download of the previous generation of this system as it is, or a plain sheet with SKU, Name, Price and Cost; the column map is in `apps/api/src/modules/products/product-csv-import.ts`. One product per stock code, updated in place on re-import (a blank cell never clears a value), rows sharing a group id or range name become a product group, unknown manufacturers are created, images are written only for a product that has none. Stock levels in the file are ignored: stock is booked in through goods received or reconciled with `scripts/import-stocktake.ts`.
- **Orders from outside come through one normalised shape.** The marketplace connectors, the admin's CSV import (native and legacy column layouts, told apart by their headings) and the API-key **order feed** (`POST /api/v1/order-feed/orders`, `GET /api/v1/order-feed/orders/:reference`, scopes `orders:write` / `orders:read`) all produce a `MarketplaceOrder` and go through `MarketplaceService.importOrders`. The sender's reference is the idempotency key; an unknown product code or warehouse refuses the whole order. `docs/ORDER-FEED.md` is the contract to hand to a caller.
...
**Google Analytics** (`lib/analytics.ts`, `components/analytics-consent.tsx`, `components/purchase-event.tsx` in **both** storefronts): GA4 Measurement ID `G-3RJWFM59VV`. Every CleverDeals storefront shares that one property — reports separate the shops by hostname — so a new shop copies the id rather than minting one. Nothing from Google loads until the visitor accepts the cookie banner (UK PECR); "Cookie settings" in the footer reopens it, and a refusal deletes the `_ga` cookies. `STORE_ANALYTICS_DISABLED=true` turns analytics and the banner off — CI's e2e job sets it so the banner cannot cover elements Playwright clicks.
- Both shops send `add_to_cart` (from `AddToCartButton`, given an `item` prop) and `purchase` (from the confirmation page). `gaEvent` is a no-op unless the tag is running, which only happens after consent, so no call site checks for it. The confirmation page retries for a few seconds — a page's effects run before its layout's — and claims the order in `localStorage` only once the event has actually gone, so a refresh never counts a second sale.
