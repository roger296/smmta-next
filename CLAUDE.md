...
- **User auth uses `@fastify/jwt`** signing with `JWT_SECRET`. **There is no users table.** Any JWT signed with the secret is accepted as-is — the API does not validate the userId/companyId payload against the DB. `apps/api/generate-test-token.ts` produces an admin-role token with the singleton `COMPANY_ID` and a random userId, valid 30 days. The admin SPA login screen literally accepts any pasted JWT.
